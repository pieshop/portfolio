...
function receiveItem(id, item, archiveItem) {
...
function _parseMedia(json, localData) {
    const {client_id, entry_id, is_dark_background, images, pdfs, swfs} = json;
    let media_path                                                      = constants.get_image_path(client_id, entry_id);
    ...
    let lst = ary.map((media_name) => {
        ...
        let o = {
            ...
            is_olm            : is_olm,
            alt               : 'image_' + client_id + '_' + entry_id
        };
...
function parseItem(json, id, clients) {
...
function parseArchiveItem(json, id, clients) {
...
function fetchItem(state, id, client_id) {
    const clients    = getLocalClientData(state);
    const hasArchive = clients[client_id][id].has_archive;
    ...
    return dispatch => {
        ...
        Promise.all(promises)
            .then((results) => {
                ...
                dispatch(receiveItem(id, parseItem(itemResult, id, clients), parseArchiveItem(archiveItemResult, id, clients)));
            })
            .catch((message) => {
...
export function fetchItemIfNeeded(id, client_id) {
