...
$("#sitemap").tablesorter({sortList:[[3,1],[1,1]],widgets:['zebra']});
$('.url').click(function(){
...
.url:hover
{
  ...
  <body>
	<div id="content">
	  	<h2>XML Sitemap</h2>
		<p class="expl">Generated by <a href="https://roumen.it/projects/laravel-sitemap/" target="_blank" title="Sitemap generator for Laravel">laravel-sitemap</a>. This sitemap contains <xsl:value-of select="count(s:urlset/s:url)"/> URLs.</p>
		...
		<tbody>
	      ...
	      <tr>
	        ...
	        <!--<td><xsl:value-of select="substring(s:lastmod,0,11)"/></td>-->
	      </tr>
	      </xsl:for-each>
		</tbody>
	    </table>
