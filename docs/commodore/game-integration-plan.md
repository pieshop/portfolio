...
- Commodore 64: `Jungle Drums` / `jungle-drums-cracked-bypass.prg`
...
commodore/c64/jungledrums/jungle-drums-original.T64
commodore/c64/jungledrums/jungle-drums-cracked-bypass.prg
...
## Known working C64 configuration

Jungle Drums is wired into the portfolio with a bypassed PRG runtime derived from the FCS source image.
...
Resolved Jungle Drums runtime details:

- Preferred runtime file: use `jungle-drums-cracked-bypass.prg`. It is derived from the FCS source image, skips the FCS crack intro, and was verified locally in VICE. `jungle-drums-original.T64` is retained as the source/reference image.
- Controls verified in browser: `A` starts the game, left/right move, up jumps, down crouches, and `X` fires while a direction key is held. `V` opens the EmulatorJS/VICE virtual keyboard and is not a game control.
- `vice_x64` is the current browser runtime core.
- Autostart uses the bypassed PRG rather than the source T64.
...
www/public/commodore/games/jungledrums/jungle-drums.prg
...
```json
{
  ...
  "core": "vice_x64",
  "emulatorCore": "vice_x64",
  "gameUrl": "/commodore/games/jungledrums/jungle-drums.prg",
  "defaultOptions": {
    "vice_reset": "autostart",
    "vice_autostart": "enabled"
  }
...
- `player.html` is generated by the copy script.
- Game-route metadata is currently derived from `portfolio.json`.
