...
class Sitemap extends Model
{
	public static function get_sitemap()
	{
		...
		if (!$sitemap->isCached()) {
			...
			$sitemap->add($sitemap_baseurl . '/all/', $update_date, '0.6', 'monthly');

			$categories = Category::get_categories_array();
			foreach ($categories as $category) {
				$category_name = $category->category_name;
				$url = $sitemap_baseurl . '/' . $category_name . '/';
			...
			foreach ($entries as $entry) {
				...
				$url = $sitemap_baseurl . '/all/' . $client_key . '/' . $entry_key;
				$sitemap->add($url, $modified_time, $priority, 'monthly');
			}
		}
//		$sitemap->model->setSloc($sitemap_baseurl.'/sitemap/styles/');
